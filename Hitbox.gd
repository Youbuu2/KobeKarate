...
func knockback(p,d,w,ks,bk,r):
	...
	base_kb = bk
	ratio = r
	return ((((((((percentage/10) + (percentage*damage/20))*(200/ (weight+100)) *1.4) +18)*(kb_scaling))+base_kb)*1))*0.015  #.004 
...
#gives angle for opponents to be launched
func angle_flipper(body):
	...
	if get_parent().direction() == -1:
		...
		xangle = (((body.global_position.angle_to_point(get_parent().global_position))*180)/PI)
	match angle_flipper:
		#sends at the exact knockback_angle every time
		0:
			body.velocity.x = (getHorizontalVelocity (knockbackVal, -angle))
			body.velocity.y = (getVerticalVelocity (knockbackVal, -angle))
			body.horizontalDecay = (getHorizontalDecay(-angle))
			body.verticleDecay = (getVerticalDecay(angle))
			# sends away from center of the enemy player
		1:
			if get_parent().direction() == -1:
			...
			body.horizontalDecay = (getHorizontalDecay(angle+180))
			body.verticleDecay = (getVerticalDecay(xangle))
			#away
			#return angle
			#sends toward center of the enemy player
		2:
			if get_parent().direction() == -1:
			...
			body.velocity.x = ((getHorizontalVelocity (knockbackVal, -xangle+180)))
			body.velocity.y = ((getVerticalVelocity (knockbackVal, -xangle)))
			body.horizontalDecay = (getHorizontalDecay(xangle+180))
			body.verticleDecay = (getVerticalDecay(xangle))
			#towards
			#return angle
			#horizontal knockback sends away from the center of the hitbox
		3:
			if get_parent().direction() == -1:
			...
			body.velocity.y = (getVerticalVelocity (knockbackVal, -angle))
			body.horizontalDecay = (getHorizontalDecay(xangle))
			body.verticleDecay = (getVerticalDecay(angle))
			#horizontal knockback sends toward the center of the hitbox
		4:
			if get_parent().direction() == -1:
				xangle = -(((body.global_position.angle_to_point(self.global_position))*180)/PI)+180
			else:
			...
			body.horizontalDecay = (getHorizontalDecay(angle))
			body.verticleDecay = (getVerticalDecay(angle))
			#horizontal knockback is reversed
		5:
			body.velocity.x = (getHorizontalVelocity (knockbackVal,angle+180))
			body.velocity.y = (getVerticalVelocity (knockbackVal, -angle))
			body.horizontalDecay = (getHorizontalDecay(angle+180))
			body.verticleDecay = (getVerticalDecay(angle))
		# horizontal knockback sends away from the enemy player
		6:
			body.velocity.x = (getHorizontalVelocity ((knockbackVal),xangle))
			body.velocity.y = (getVerticalVelocity (knockbackVal, -angle))
			body.horizontalDecay = (getHorizontalDecay(xangle))
			body.verticleDecay = (getVerticalDecay(angle))
			#away
			#horizontal knockback sends toward the enemy player
		7:
			body.velocity.x = (getHorizontalVelocity (knockbackVal,-xangle+180))
			body.velocity.y = (getVerticalVelocity (knockbackVal, -angle))
			body.horizontalDecay = (getHorizontalDecay(angle))
