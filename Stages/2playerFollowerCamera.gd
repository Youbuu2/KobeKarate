...
# Adjusts the zoom level to keep all objects in view
# and optionally displays debug information

# Variables for controlling zoom and debug mode

export(float, 0.1, 0.5) var zoom_offset : float = 0.2
export var debug_mode : bool = false

# Rectangles to represent camera and viewport areas
...
func calculate_zoom(rect: Rect2, viewport_size: Vector2) -> Vector2:
		# Calculates zoom level based on camera and viewport sizes
	var max_zoom = max(
...
func _draw() -> void:
		# Draw camera rectangle and center point if in debug mode
	if not debug_mode:
		return
